...
DELETION_INTERVAL       =   60          #[seconds] how often check to delete data from database
...
class DataGenerator:
    ...
    def data_generation_loop(self,_L):
    ...
    def gen_thread_target(self, _L):
        ...
        while self.running:
            ...
            #Generate new data.
            temp_output = self.data_generation_loop(_L)
    ...
    #TO DO: Update so will shutdown faster! currently only shuts down after DELETION_INTERVAL, so may take awhile for thread to join().
    def del_thread_target(self,_L):
        while self.running:
            time.sleep(DELETION_INTERVAL)
            with _L: #BLOCKING, it's ok to wait to delete data.
                self.database.delete_acknowledged()
