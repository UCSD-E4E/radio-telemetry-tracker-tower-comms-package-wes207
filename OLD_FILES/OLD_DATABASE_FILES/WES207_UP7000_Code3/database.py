...
ACK_IDX         =   7   #int        Not really needed, can always assume it's not acknowledged when added as record.
...
class TowerDatabase:
    ...
    # METHODS in class:
    # 1) initialize the db (internal use only), 2) add a new data record, 3) Mark record ack
    # 4) delete all ack'd records, 5) get most recent sample index, 6) print the current database
    # 7) print a list of all unack'd records
    ...
    def add_record(self,records):
        ...
        cur.execute('''INSERT INTO records 
                    ...
                    ''', (
                        ...
                        int(records[DEVICE_IDX]),
                        int(records[SAMPLE_IDX]),
                        str(records[TIME_IDX]),
                        float(records[LAT_IDX]),
                        float(records[LON_IDX]),
                        float(records[ALT_IDX]),
                        0 #assume zero, but could be int(record[ACK_IDX])
                    ))
    ...
    #3 mark all records with ack=1
    def mark_ack(self, sample_nums):
        ...
        num_rows_updated = cur.rowcount #number of rows that were updated in database
        print(f"Ack for {num_rows_updated} variables set to 1.")
        con.commit()
        con.close()

    #4 Delete ack'd records from db
    def delete_acknowledged(self):
        con = sqlite3.connect(self.db_name)
        cur = con.cursor()
        
        # Delete acknowledged records
        cur.execute('DELETE FROM records WHERE ack = 1')
    ...
    #5 Get the most recent sample from db
    ...
    #6 Output entire database (for debugging only)
    ...
    #7 print list of all unack'd records (for debugging only)
