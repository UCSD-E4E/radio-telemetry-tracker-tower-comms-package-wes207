...
class LoRaCommunicator:
    def __init__(self, database_lock):
        ...
        #Sending record deletion & timeout
        self.record_requested = False                   #has a record been requested?
        self.waiting_for_ack = False
        self.last_sent_was_record = False               #If most recent request was record, then we can delete record from db with ack.
        self.last_record_send_time = 0                  #for timeout to set self.last_sent_was_record to False.
        self.record_timeout = 0.750                     #seconds
        self.record_request_time = 0                    #for timeout of record_requested.
        self.record_request_timeout = 5.0               #seconds. Timeout for record_requested to be flagged false.
        #ack timeout logic
        self.ack_timeout = 1.0                          # seconds to wait for ACK
        self.send_interval = 2.0                        # seconds between attempts to send
        self.last_send_time = 0                         # Timestamp of last send attempt


        #Set port for serial comms with heltec based on OS/port. DO THIS UPON SETUP!
        self.port = 'COM4'                              # Serial port (windows), use device manager to change port to COMXX
        #self.port = '/dev/ttyUSB0'                     # Serial port on Ubuntu. $ls /dev/tty* 
        #self.port = '/dev/tty.usbserial-2'             # Serial port for mac

        # Statistics for shutdown monitoring to see how many records ack'd vs crc errors
        self.records_sent = 0
    ...
    def _main_send_loop(self):
        ...
        while self.running:
            try:
                ...
                #timeout for last_sent_was_record
                if self.last_sent_was_record and (current_time - self.last_record_send_time > self.record_timeout):
                    print(f"LoRa: last_sent_was_record timeout for record. ")
                    self.last_sent_was_record = False
                    #self.timeouts += 1

                # timeout for waiting_for_ack
                if self.waiting_for_ack and (current_time - self.last_send_time > self.ack_timeout):
                    ...
                    if self.database_lock.locked():
                        try:
                            ...
                            print(f"Released database lock after timeout of {self.ack_timeout} seconds.")
                ...
                #timeout for rec? command
                if self.record_requested and (current_time - self.record_request_time > self.record_request_timeout):
                    print("LoRa: Record request expired.")
                    self.record_requested = False

                # Send a record here.
                # Only try to send if not waiting for ACK and enough time has passed. Also only send if record_requested=True.
                if not self.waiting_for_ack:
                    if self.record_requested and (current_time - self.last_send_time >= self.send_interval):
                        if self._send_next_record():
                            self.last_send_time = current_time
                            self.record_requested = False
                        else:
                            print("Failed to send record.")
                    #elif not self.record_requested:
                        #print("LoRa: Waiting for rec? before sending next record.") #this will spam terminal output...
                else:
                    print("LoRa: Waiting for ack so no record was sent.")
        ...
        try:
            ...
            try:
                ...
                if record_string and sample_num:
                    ...
                    # Send the record and set characteristics for deleting from database.
                    if self._send_record(record_string):
                        self.pending_record = record_string
                        self.pending_sample_num = sample_num
                        self.waiting_for_ack = True
                        self.last_sent_was_record = True
                        self.last_record_send_time = time.time()
                        self.records_sent += 1
    ...
    def _process_received_message(self, message):
        ...
        try:
            ...
            if message.startswith("rec?"):
                ...
                self.record_request_time = time.time()
    ...
    def _handle_ack(self):
        ...
        try:
            if self.waiting_for_ack and self.pending_sample_num and self.last_sent_was_record:
                ...
                print("LoRa: Received ACK but no pending record to delete.")
