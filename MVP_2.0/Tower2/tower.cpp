...
#define ADC_ATTENDB  ADC_ATTEN_DB_11 // Full range: 0 - 3.3V
#define ADC_UNIT     ADC_UNIT_1
#define VREF         3300  // mV, obtained from esp_adc_cal_characterize, or set to 3300mV.
...
uint32_t read_adc_voltage() {
  ...
  uint32_t voltage = (uint32_t)((float)adc_value * VREF / 4095.0); // 4095 for 12-bit
  return voltage;
}
...
#define JN_LORA_SPREAD   7     
#define JN_LORA_CR       5 //5-8
#define JN_LORA_BW       125.0 
...
/*====== My globals =====*/
String last_packet;
char extractedFields_ota[4][128]; // = {'\0'}; // Assuming a maximum of 4 fields, each max 128 chars + null
...
//see if bitNumber specified is equal to 1
    ...
    switch (getCommandType(command)) {
      ...
      case Q_IDN:
        if (numFields>=3){
          ...
          if (device_id == DEVICE_ID)
            int16_t res = Tx(String(DEVICE_TYPE)+", ID: "+String(DEVICE_ID)+", Firmware version: "+String(FW_VER)+", Channels: Uplink: "+String(CHANNEL_UPLINK)+ "/ Downlink: "+String(CHANNEL_DOWNLINK));
      ...
      case CMD_RLY:
          if (numFields>=3){
            int device_id = atoi(extractedFields_ota[1]);
            int stat = atoi(extractedFields_ota[2]);
            if (device_id == DEVICE_ID){
              if (stat == 1){
                ...
                cb(&SM, RLY);
			        ...
			        int16_t res = Tx("Rly set to: "+String(extractedFields_ota[2]));
            }             
          }
          break;
       case Q_DOUT:
      ...
      case CMD_DOUT:
          if (numFields>=3){
            ...
            if (device_id == DEVICE_ID){
              ...
              int16_t res = Tx("DOUT set to: "+String(extractedFields_ota[2]));             
            }             
          }
          break;            
      case Q_ADC:
      ...
      case CMD_RST:
          if (numFields>=2){
            int device_id = atoi(extractedFields_ota[1]);
            if (device_id == DEVICE_ID){
              int16_t res = Tx("Reset (re-initialize device): "+String(device_id));   
              //~ call for reinit function here and reset ST bitmap       
            }             
          }
          break;
      case CMD_ID:
        if (numFields>=2){
          int device_id = atoi(extractedFields_ota[1]);
          int newValue = atoi(extractedFields_ota[2]);
          if (device_id == DEVICE_ID)
...
void parse_s_command(const char *command){
  switch (getCommandType(command)) {
    case Q_IDN:
      if (numFields>=1){
        Serial.println(String(DEVICE_TYPE)+", ID: "+String(DEVICE_ID)+", Firmware version: "+String(FW_VER));
        delay(100);
      }
      break;     
    default:
...
void handle_button_press(){
    ...
    int16_t state = Tx(s);
    String msg = String(DEVICE_TYPE)+", ID: "+String(DEVICE_ID)+", Firmware version: "+String(FW_VER)+", Uplink: "+String(CHANNEL_UPLINK)+ "/ Downlink: "+String(CHANNEL_DOWNLINK);   
    /*
    String msg = String(DEVICE_TYPE)+", ID: "+String(DEVICE_ID)+", Firmware version: "+String(FW_VER)+", Uplink: "+String(CHANNEL_UPLINK)+ "/ Downlink: "+String(CHANNEL_DOWNLINK);   
    int16_t state = Tx(msg);Serial.println(msg);	
    */
	if (state == RADIOLIB_ERR_NONE) {
      Serial.println(msg);
    } else if (state == RADIOLIB_ERR_PACKET_TOO_LONG) {
